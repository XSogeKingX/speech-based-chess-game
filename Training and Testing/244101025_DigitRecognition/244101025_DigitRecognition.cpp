// 244101025_DigitRecognition.cpp : Defines the entry point for the console application.
//
...
void testing()
{
	...
	for(int i=0;i<=9;i++)
	{
		...
		for(int j=31;j<=40;j++)
		{
			sprintf(ch,"./Input_Testing/244101025_%d_%d.txt",i,j);
			FILE *fp=fopen(ch,"r");
			if(fp==NULL)
			{
			...
			printf("Predicted Output for file : %s --> Digit %d\n",ch,indx);
		}
		printf("\nAccuracy of %d is: %d\n\n",i,accuracy_count*10);
		overall_accuracy_count+=accuracy_count;
	}
	printf("Total Model Accuracy : %d%% \n",overall_accuracy_count);
}
...
void training()
{
	...
	for(int i=0;i<=9;i++)
	{
		...
		for(int j=1;j<=sc;j++)
		{
			sprintf(ch,"Input_Training/244101025_%d_%d.txt",i,j);
			FILE *fp=fopen(ch,"r");
			if(fp==NULL)
			{
				printf("Cannot Open File\n");
